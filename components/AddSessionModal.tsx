...
interface AddSessionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (sessionData: Omit<EngagementSession, 'id' | 'status'>) => void;
  initialData?: EngagementSession | null;
}

type SessionFormData = Omit<EngagementSession, 'id' | 'status' | 'startTime' | 'endTime'> & {
  date: string;
  startTime: string;
  endTime: string;
};

const AddSessionModal: React.FC<AddSessionModalProps> = ({ isOpen, onClose, onSubmit, initialData }) => {
  const [formData, setFormData] = useState<SessionFormData>({
    title: '',
    category: 'General Catch-up',
    date: '',
    startTime: '',
    endTime: '',
    agenda: '',
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4" aria-modal="true" role="dialog">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-lg font-heading font-semibold text-r-gray-900">{initialData ? 'Reschedule Session' : 'Add New Session'}</h2>
          ...
          <div className="p-6 space-y-4">
            ...
            <div>
                 ...
                 <select name="category" id="category" value={formData.category} onChange={handleInputChange} required className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-r-gray-300 focus:outline-none focus:ring-r-blue focus:border-r-blue sm:text-sm rounded-md">
                    ...
                    <option>General Catch-up</option>
                 </select>
            </div>
             <div>
