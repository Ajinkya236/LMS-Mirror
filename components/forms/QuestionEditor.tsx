...
export const QuestionEditor: React.FC<QuestionEditorProps> = ({
  ...
  // Bulk options adder state
  const [showBulkModal, setShowBulkModal] = useState(false);
  const [bulkText, setBulkText] = useState('');

  // Sync with global collapse/expand control
  useEffect(() => {
  ...
  const handleTypeChange = (newType: QuestionType) => {
    ...
    if (newType === 'choice' || newType === 'single_choice' || newType === 'multiple_choice') {
      ...
      if (!question.choiceDisplay) {
        updates.choiceDisplay = newType === 'multiple_choice' ? 'checkbox' : 'radio';
      }
      updates.multipleAnswers = newType === 'multiple_choice';
  ...
  // Bulk add option processor
  const handleApplyBulkOptions = (mode: 'replace' | 'append') => {
    ...
    const generated: QuestionOption[] = lines.map((text, idx) => ({
      id: `opt-${Date.now()}-${idx}-${Math.random().toString(36).substr(2, 4)}`,
      text
    }));

    const finalOptions = mode === 'replace' ? generated : [...(question.options || []), ...generated];
    const firstId = finalOptions[0]?.id;

    onUpdate({
      ...question,
      options: finalOptions,
      correctOptionId: isQuiz && (!question.correctOptionId || mode === 'replace') ? firstId : question.correctOptionId
    });
  ...
  const currentSection = sections.find(s => s.id === question.sectionId) || sections[0];

  // Helper type label
  const getTypeBadgeLabel = (type: QuestionType) => {
    switch (type) {
      case 'rating': return '5-Star Rating';
      case 'likert': return 'Likert Scale';
      case 'single_choice': return 'Single Choice';
      case 'multiple_choice': return 'Multiple Choice';
      case 'choice': return question.multipleAnswers ? 'Multiple Choice' : 'Single Choice';
      case 'nps': return 'NPS (0-10)';
      case 'long_text': return 'Long Feedback';
    ...
    <div className="bg-white border border-slate-200 hover:border-slate-300 rounded-2xl shadow-xs transition-all duration-200 overflow-hidden">
      ...
      <div className="bg-slate-50/90 px-4 py-3 border-b border-slate-200 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <select
            value={
              question.type === 'single_choice' || question.type === 'multiple_choice'
                ? (question.multipleAnswers ? 'multiple_choice' : 'single_choice')
          ...
          >
            ...
            <option value="rating">Rating (Star / Scale)</option>
            ...
            <option value="long_text">Long Feedback (Paragraph)</option>
          ...
          {sections.length > 0 && (
            <div className="flex items-center gap-1">
              ...
              <select
                ...
                className="px-2 py-1 bg-slate-100/80 border border-slate-200 rounded-lg text-[11px] font-semibold text-slate-700 max-w-[150px] truncate"
              >
        ...
        <div className="flex items-center gap-1">
          ...
          {(question.type === 'choice' || question.type === 'single_choice') && (
            <button
              ...
              title="Configure conditional branching rules"
            >
              <GitBranch className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">Branching {branchingRuleCount > 0 ? `(${branchingRuleCount})` : ''}</span>
            </button>
          )}
          ...
          <button
            ...
            className="p-1.5 text-slate-500 hover:text-slate-800 hover:bg-slate-200/60 rounded-md"
          ...
          <button
            ...
            disabled={index === 0}
            onClick={onMoveUp}
            className="p-1.5 text-slate-500 hover:text-slate-800 disabled:text-slate-300 disabled:cursor-not-allowed hover:bg-slate-200/60 rounded-md"
      ...
      {isCollapsed ? (
        <div
          ...
          className="p-4 cursor-pointer hover:bg-slate-50/50 flex items-center justify-between gap-4 transition-colors"
        >
          <div className="flex items-center gap-2.5 truncate">
            ...
            {question.required && (
              <span className="px-1.5 py-0.5 bg-rose-50 text-rose-600 rounded text-[10px] font-bold shrink-0">
                Required
              </span>
            )}
            {isQuiz && question.points && (
              <span className="px-1.5 py-0.5 bg-amber-50 text-amber-800 rounded text-[10px] font-bold shrink-0">
                {question.points} pts
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 text-xs text-slate-400 font-semibold shrink-0">
            <span>{getTypeBadgeLabel(question.type)}</span>
            <span>•</span>
          ...
          <div className="space-y-2">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Select the primary specification from the options below."
            ...
            <div className="space-y-3 pt-1">
              ...
              <div className="space-y-2">
                <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center justify-between">
                  <span>Options {isQuiz ? '(Select Correct Answer Key)' : ''}</span>
                  <span className="text-[10px] text-slate-400 font-normal">
                    {isQuiz ? 'Checkmark designates the scoring answer' : ''}
                  </span>
                </div>
                ...
                {/* Buttons: Add Single Option & Bulk Add Options */}
                <div className="flex items-center gap-2 pt-1">
                  <button
                    type="button"
                    onClick={handleAddOption}
                    className="px-3 py-1.5 text-xs font-bold text-indigo-600 bg-indigo-50/60 hover:bg-indigo-100 rounded-xl flex items-center gap-1.5 transition-colors"
                  >
                ...
                {/* Bulk Options Slide-Down Box */}
                {showBulkModal && (
                  <div className="p-3.5 bg-slate-50 border border-indigo-200 rounded-2xl space-y-2.5 animate-fade-in">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-slate-800">
                        Paste Multiple Options (1 per line)
                      </span>
                      <button
                        type="button"
                    ...
                    <textarea
                      ...
                      className="w-full p-2.5 bg-white border border-slate-300 rounded-xl text-xs font-mono text-slate-800 focus:outline-none focus:border-indigo-500"
                    />
                    <div className="flex items-center justify-end gap-2">
                      <button
                        type="button"
                        onClick={() => handleApplyBulkOptions('append')}
                        className="px-3 py-1.5 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-xl text-xs font-bold"
                      >
                        Append to Existing
                      </button>
                      <button
                        type="button"
                        onClick={() => handleApplyBulkOptions('replace')}
                        className="px-3 py-1.5 bg-indigo-600 text-white hover:bg-indigo-700 rounded-xl text-xs font-bold"
                      >
                        Replace All Options
                      </button>
                    </div>
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-bold text-slate-700">Rating Scale:</span>
                    <div className="flex items-center gap-1">
                      {[3, 4, 5, 7, 10].map((lvl) => (
                        <button
                          key={lvl}
