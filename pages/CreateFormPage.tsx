...
export const CreateFormPage: React.FC = () => {
  ...
  // Navigation tab ('info' | 'questions' | 'settings' | 'preview')
  ...
  useEffect(() => {
    if (formId) {
      ...
      if (found) {
        ...
        setCategory(found.category || 'Training Evaluation');
        setTargetAudience(found.targetAudience || 'All Organization');
        ...
        setEndDate(found.endDate || found.settings?.endDate || '');
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-28">
              ...
              <div className="flex items-center gap-3 pt-1">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    Design custom evaluation questions, quiz assessments, branching logic, and automated scoring.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between overflow-x-auto">
              <div className="flex items-center gap-1 py-1">
                {[
                  { id: 'info', label: '1. Form Details', icon: FileText },
          ...
          <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Choose the structural behavior for this evaluation instrument.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {[
                  ...
                  return (
                    ...
                    >
                      {isSelected && (
                        <span className="absolute top-3 right-3 p-1 rounded-full bg-r-blue text-white">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 focus:outline-none focus:border-r-blue"
                  >
        ...
        {activeTab === 'questions' && (
          <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
              ...
              {displayedQuestions.length === 0 && (
                <div className="p-8 text-center bg-white rounded-2xl border border-dashed border-slate-300">
                  ...
                  <button
                    type="button"
                    onClick={() => handleAddQuestion(selectedSectionFilter !== 'all' ? selectedSectionFilter : undefined)}
                    className="mt-3 px-4 py-2 bg-r-blue text-white rounded-xl text-xs font-bold"
                  >
            ...
            <div className="flex items-center justify-between pt-4">
              ...
              >
                <span>Proceed to Settings & Behavior</span>
                <ArrowRight className="w-4 h-4" />
              </button>
            </div>
          </div>
        ...
        {activeTab === 'preview' && (
          <div className="max-w-2xl mx-auto space-y-5 animate-fade-in">
            <div className="bg-slate-900 text-white p-4 rounded-2xl flex items-center justify-between shadow-xs">
              <div className="flex items-center gap-2">
                <Eye className="w-4 h-4 text-r-blue-200 text-blue-300" />
                    ...
                    .map((q, idx) => (
                      <div key={q.id} className="space-y-2 p-4 bg-slate-50/70 rounded-2xl border border-slate-200">
                        <div className="flex items-start gap-2">
                          <span className="text-xs font-bold text-r-blue mt-0.5">{idx + 1}.</span>
                          <div>
                                ...
                                <label
                                  ...
                                  className={`flex items-center gap-3 p-2.5 rounded-xl border text-xs font-semibold transition-all cursor-pointer ${
                                    previewAnswers[q.id] === opt.id
                                      ? 'bg-blue-50 border-r-blue text-blue-950'
                  ...
                  <div className="pt-4 flex items-center justify-between">
                    ...
                    {previewSectionIndex < sections.length - 1 ? (
                      <button
                        type="button"
                        onClick={() => setPreviewSectionIndex(prev => prev + 1)}
                        className="px-5 py-2 bg-r-blue hover:bg-blue-700 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-md transition-colors"
                      >
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-200 py-3 px-4 sm:px-8 shadow-lg">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
          {/* Status / Errors Info */}
          <div className="flex items-center gap-3">
            <span className="text-xs font-bold text-slate-600">
              {activeTab === 'info' && 'Step 1 of 4: Form Details & Archetype'}
              {activeTab === 'questions' && `Step 2 of 4: Questionnaire Architecture (${questions.length} Questions)`}
              {activeTab === 'settings' && 'Step 3 of 4: Scheduling & Submission Rules'}
              {activeTab === 'preview' && 'Step 4 of 4: Live Respondent Simulator'}
            </span>

            {Object.keys(errors).length > 0 && (
              <span className="text-xs font-bold text-rose-600 bg-rose-50 px-2.5 py-1 rounded-lg border border-rose-200 flex items-center gap-1 animate-pulse">
                <AlertCircle className="w-3.5 h-3.5" />
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => handleSave(false)}
              className="px-4 py-2 rounded-xl text-xs font-bold bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 flex items-center gap-1.5 transition-colors"
            >
              <Save className="w-3.5 h-3.5" />
              <span>Save Draft</span>
            </button>
            ...
            <button
              type="button"
              onClick={() => handleSave(true)}
              className="px-5 py-2 rounded-xl text-xs font-bold bg-r-blue hover:bg-blue-700 text-white flex items-center gap-1.5 transition-colors shadow-sm"
            >
              <Send className="w-3.5 h-3.5" />
              <span>{isEditing && existingForm?.status === 'Published' ? 'Update & Keep Live' : 'Publish Form'}</span>
            </button>
